...
case class JavaRandomState[T](n: Long, g: Long => T) extends RandomState[T] {
  ...
  def next: RandomState[T] = {
// TO BE IMPLEMENTED
val newSeed = new util.Random(n).nextLong()
    JavaRandomState(newSeed, g)
  }
...
object RandomState {
  ...
  // 4 points
  val longToDouble: Long => Double ={x=>
    val longDif=Long.MaxValue.toDouble-Long.MinValue.toDouble
    val scale=(x-Long.MinValue.toDouble)/longDif*2.0+(-1.0)
    scale
  }
// TO BE IMPLEMENTED 
